...
import torch
from dataReader_utils.dataReader import WikiCS_loader
...
if __name__ == '__main__':
    ...
    graph, features, labels, n_classes, train_mask, test_mask, valid_mask = WikiCS_loader(self_loops, device)
                    ...
                    for run in run_list:
                        ...
                        if not os.path.exists(training_log_dir):
                            ...
                            model_impl.train_test_model(input_features=features,
                                                        labels=labels,
                                                        train_mask=train_mask[run],
                                                        test_mask=test_mask,
                                                        valid_mask=valid_mask[run],
                                                        n_epochs=n_epochs,
                                                        test_epoch=test_epoch,
                                                        test_name=test_name,
